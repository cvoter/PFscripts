function [ ] = outputsCalculateMatlabDZ()
...
elseif strcmp(flux,'subsurface_storage') == 1
    ...
    load(strcat(GHOME,'/press.grid.step.mat')); p = data; clear data;
    specificStorage = 1.0e-4;
    varDZ(1) = 2*(z(1) - 0);
    for k = 2:length(z)
        varDZ(k) = 2*(z(k) - (z(k-1)+varDZ(k-1)/2));
    end
    for t = 1:length(sat)
        thisSat = sat{t};
        thisPress = p{t};
        for k = 1:length(z)
            thisSubStorage(:,:,k) = thisSat(:,:,k)*dx*dy*varDZ(k).*(specificStorage*thisPress(:,:,k) + porosity_soil);
        end
...
if strcmp(flux,'surface_storage') ~= 1 && strcmp(flux,'subsurface_storage') ~= 1
