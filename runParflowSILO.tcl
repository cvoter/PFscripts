...
#------------------------------------------------------------------------------------------
# Starting Message with time and parameter check
#------------------------------------------------------------------------------------------
set startTime [clock seconds]
set time [clock format [clock seconds] -format "%H:%M:%S %Y-%m-%d"]
puts $time
puts [format "Starting ParFlow at Hour #%d" $pfStartCount]
puts [format "   TIMING"]
puts [format "   Stop Time = %.0f hr" $pfStopTime]
puts [format "   DOMAIN"]
puts [format "   (xL,yL,zL) = (%.2f, %.2f, %.2f) meters" $xL $yL $zL]
puts [format "   (nx,ny,nz) = (%.0f, %.0f, %.0f)" $nx $ny $nz]
puts [format "   (dx,dy,dz) = (%.2f, %.2f, %.2f) meters" $dx $dy $dz]
puts [format "   (xU,yU,zU) = (%.2f, %.2f, %.2f) meters" $xU $yU $zU]
puts [format "   (P,Q,R,np) = (%.0f, %.0f, %.0f, %.0f)" $ptP $ptQ $ptR $np]
puts [format "   SOIL"]
puts [format "   (Ksat,mn) = (%.4e, %.4e)" $Ks_soil $mn_grass]
puts [format "   (VGa,VGn) = (%.2f, %.2f)" $VGa_soil $VGn_soil]
puts [format "   (porosity,Ssat,Sres) = (%.2f, %.2f, %.3f)" $porosity_soil $Ssat_soil $Sres_soil]
puts [format "   IMPERVIOUS"]
puts [format "   (Ksat,mn) = (%.4e, %.4e)" $Ks_imperv $mn_imperv]
puts [format "   (VGa,VGn) = (%.2f, %.2f)" $VGa_imperv $VGn_imperv]
puts [format "   (porosity,Ssat,Sres) = (%.3f, %.2f, %.3f)" $porosity_imperv $Ssat_imperv $Sres_imperv ]
